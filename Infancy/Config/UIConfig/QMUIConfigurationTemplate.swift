 ...
 *  1. 在项目代码的文件夹里找到 QMUIConfigurationTemplate 目录，将QMUIConfigurationTemplate复制，添加新的配置颜色
 *  2. 在自己项目的 AppDelegate 里，在 application:didFinishLaunchingWithOptions: 里调用 [QMUIConfigurationTemplate setupConfigurationTemplate]，即可让配置表生效。
...
let QMUICMI = QMUIConfiguration.sharedInstance()

struct QMUIConfigurationTemplate {
    ...
    static func setupConfigurationTemplate(){
        ...
        QMUICMI?.whiteColor = UIHelp.UIColorMake(255, 255, 255)  // UIColorWhite : 白色（不用 [UIColor whiteColor] 是希望保持颜色空间为 RGB）
        QMUICMI?.blackColor = UIHelp.UIColorMake(0, 0, 0    )// UIColorBlack : 黑色（不用 [UIColor blackColor] 是希望保持颜色空间为 RGB）
        QMUICMI?.grayColor = UIHelp.UIColorMake(179, 179, 179)                             // UIColorGray  : 最常用的灰色
        QMUICMI?.grayDarkenColor = UIHelp.UIColorMake(163, 163, 163)                       // UIColorGrayDarken : 深一点的灰色
        QMUICMI?.grayLightenColor = UIHelp.UIColorMake(198, 198, 198)                      // UIColorGrayLighten : 浅一点的灰色
        QMUICMI?.redColor = UIHelp.UIColorMake(250, 58, 58)                                // UIColorRed : 红色
        QMUICMI?.greenColor = UIHelp.UIColorMake(159, 214, 97)                             // UIColorGreen : 绿色
        QMUICMI?.blueColor = UIHelp.UIColorMake(49, 189, 243)                              // UIColorBlue : 蓝色
        QMUICMI?.yellowColor = UIHelp.UIColorMake(255, 207, 71)                            // UIColorYellow : 黄色
        
        QMUICMI?.linkColor = UIHelp.UIColorMake(56, 116, 171)                              // UIColorLink : 文字链接颜色
        ...
        QMUICMI?.maskDarkColor = UIHelp.UIColorMake(0, 0, 0, 0.35)                 // UIColorMask : 深色的背景遮罩，默认用于 QMAlertController、QMUIDialogViewController 等弹出控件的遮罩
        QMUICMI?.maskLightColor = UIHelp.UIColorMake(255, 255, 255, 0.5)           // UIColorMaskWhite : 浅色的背景遮罩，QMUIKit 里默认没用到，只是占个位
        QMUICMI?.separatorColor = UIHelp.UIColorMake(222, 224, 226)                        // UIColorSeparator : 全局默认的分割线颜色，默认用于列表分隔线颜色、UIView (QMUI_Border) 分隔线颜色
        QMUICMI?.separatorDashedColor = UIHelp.UIColorMake(17, 17, 17)                     // UIColorSeparatorDashed : 全局默认的虚线分隔线的颜色，默认 QMUIKit 暂时没用到
        QMUICMI?.placeholderColor = UIHelp.UIColorMake(196, 200, 208)                      // UIColorPlaceholder，全局的输入框的 placeholder 颜色，默认用于 QMUITextField、QMUITextView，不影响系统 UIKit 的输入框
        
        // 测试用的颜色
        QMUICMI?.testColorRed = UIHelp.UIColorMake(255, 0, 0, 0.3)
        QMUICMI?.testColorGreen = UIHelp.UIColorMake(0, 255, 0, 0.3)
        QMUICMI?.testColorBlue = UIHelp.UIColorMake(0, 0, 255,0.3)
        ...
        //MARK: - UIButton
        QMUICMI?.buttonHighlightedAlpha = QMUICMI?.controlHighlightedAlpha ?? 0              // ButtonHighlightedAlpha : QMUIButton 在 highlighted 时的 alpha，不影响系统的 UIButton
        QMUICMI?.buttonDisabledAlpha = QMUICMI?.controlDisabledAlpha ?? 0                      // ButtonDisabledAlpha : QMUIButton 在 disabled 时的 alpha，不影响系统的 UIButton
        ...
        QMUICMI?.navBarButtonFont = nil                                             // NavBarButtonFont : QMUINavigationButtonTypefalsermal 的字体（由于系统存在一些 bug，这个属性默认不对 UIBarButtonItem 生效）
        ...
        QMUICMI?.navBarBarTintColor = QMUICMI?.redColor                                          // NavBarBarTintColor : UINavigationBar.barTintColor，也即背景色
        QMUICMI?.navBarTintColor = QMUICMI?.whiteColor                                              // NavBarTintColor : QMUINavigationBar 的 tintColor，也即导航栏上面的按钮颜色
        ...
        QMUICMI?.navBarTitleColor = nil                                        // NavBarLargeTitleColor : UINavigationBar 在大标题模式下的标题颜色，仅在 iOS 11 之后才有效
        QMUICMI?.navBarTitleFont = nil                                         // NavBarLargeTitleFont : UINavigationBar 在大标题模式下的标题字体，仅在 iOS 11 之后才有效
...
//        QMUICMI?.navBarCloseButtonImage = [UIImage qmui_imageWithShape:QMUIImageShapeNavClose size:CGSizeMake(16, 16) tintColor:NavBarTintColor]     // NavBarCloseButtonImage : QMUINavigationButton 用到的 × 的按钮图片
        ...
        QMUICMI?.navBarActivityIndicatorViewStyle = UIActivityIndicatorViewStyle.gray// NavBarActivityIndicatorViewStyle : QMUINavigationTitleView 里左边 loading 的主题
//        QMUICMI?.navBarAccessoryViewTypeDisclosureIndicatorImage = [UIImage qmui_imageWithShape:QMUIImageShapeTriangle size:CGSizeMake(8, 5) tintColor:UIColorWhite]     // NavBarAccessoryViewTypeDisclosureIndicatorImage : QMUINavigationTitleView 右边箭头的图片
        ...
        QMUICMI?.tabBarItemTitleColorSelected = QMUICMI?.tabBarTintColor                    // TabBarItemTitleColorSelected : 选中的 UITabBarItem 的标题颜色
        ...
        QMUICMI?.toolBarTintColor = nil                                             // ToolBarTintColor : UIToolbar 的 tintColor，以及 QMUIToolbarButton falsermal 状态下的文字颜色
...
//        QMUICMI?.toolBarTintColorDisabled = [ToolBarTintColor colorWithAlphaComponent:ToolBarDisabledAlpha]         // ToolBarTintColorDisabled : QMUIToolbarButton 在 disabled 状态下的文字颜色
        ...
        QMUICMI?.searchBarPlaceholderColor = QMUICMI?.placeholderColor                     // SearchBarPlaceholderColor : QMUISearchBar 里的文本框的 placeholder 颜色
        ...
        QMUICMI?.tableViewEstimatedHeightEnabled = true                              // TableViewEstimatedHeightEnabled : 是否要开启全局 UITableView 的 estimatedRow(Section/Footer)Height
        ...
        QMUICMI?.tableViewSeparatorColor = QMUICMI?.separatorColor                         // TableViewSeparatorColor : 列表的分隔线颜色
        
        QMUICMI?.tableViewCellNormalHeight = 44                                     // TableViewCellfalsermalHeight : 列表默认的 cell 高度
        ...
        QMUICMI?.tableViewCellSelectedBackgroundColor = UIHelp.UIColorMake(238, 239, 241)  // TableViewCellSelectedBackgroundColor : QMUITableViewCell 点击时的背景色
        ...
        QMUICMI?.tableViewSectionHeaderBackgroundColor = UIHelp.UIColorMake(244, 244, 244)                         // TableViewSectionHeaderBackgroundColor : Plain 类型的 QMUITableView sectionHeader 的背景色
        QMUICMI?.tableViewSectionFooterBackgroundColor = UIHelp.UIColorMake(244, 244, 244)                         // TableViewSectionFooterBackgroundColor : Plain 类型的 QMUITableView sectionFooter 的背景色
        QMUICMI?.tableViewSectionHeaderFont = UIFont.boldSystemFont(ofSize: 12)                                            // TableViewSectionHeaderFont : Plain 类型的 QMUITableView sectionHeader 里的文字字体
        QMUICMI?.tableViewSectionFooterFont = UIFont.boldSystemFont(ofSize: 12)                                           // TableViewSectionFooterFont : Plain 类型的 QMUITableView sectionFooter 里的文字字体
        ...
        QMUICMI?.tableViewGroupedSectionHeaderFont = UIFont.systemFont(ofSize: 12)                                        // TableViewGroupedSectionHeaderFont : Grouped 类型的 QMUITableView sectionHeader 里的文字字体
        QMUICMI?.tableViewGroupedSectionFooterFont = UIFont.systemFont(ofSize: 12)                                       // TableViewGroupedSectionFooterFont : Grouped 类型的 QMUITableView sectionFooter 里的文字字体
        ...
        QMUICMI?.supportedOrientationMask = UIInterfaceOrientationMask.portrait      // SupportedOrientationMask : 默认支持的横竖屏方向
        QMUICMI?.automaticallyRotateDeviceOrientation = false                          // AutomaticallyRotateDeviceOrientation : 是否在界面切换或 viewController.supportedOrientationMask 发生变化时自动旋转屏幕
        QMUICMI?.statusbarStyleLightInitially = false                                  // StatusbarStyleLightInitially : 默认的状态栏内容是否使用白色，默认为 false，也即黑色
        QMUICMI?.needsBackBarButtonItemTitle = false                                   // NeedsBackBarButtonItemTitle : 全局是否需要返回按钮的 title，不需要则只显示一个返回image
        QMUICMI?.hidesBottomBarWhenPushedInitially = false                             // HidesBottomBarWhenPushedInitially : QMUICommonViewController.hidesBottomBarWhenPushed 的初始值，默认为 false，以保持与系统默认值一致，但通常建议改为 true，因为一般只有 tabBar 首页那几个界面要求为 false
